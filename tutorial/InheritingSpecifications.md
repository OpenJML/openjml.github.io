...
In classic object-oriented design, a base class, perhaps abstract, can be used without knowing what specific kind of derived class an objectt dynamically is.
One can talk about a Shape's perimeter or area without needing to know what kind of Shape it is.
When we write specifications for the base class we define the general behavior and then any derived class is expected to adhere to that behavior.
A derived class may have additional, more specific, behavior.
...
properties: a polygon has at least three sides. Then `Square` is a concrete class that implements
...
* The method `sides()` in `Square` must satisfy each of its behaviors independently, so they cannot contradict each other
...
However their specifications will be checked when verifying any derived class.
