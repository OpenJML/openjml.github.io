...
One potential disadvantage of custom-designed types is that it is an additional sub-language for the JML user to use. However, a library of Java-based classes for specification seems to require the same amount of learning.
...
* [`\bigint`](type-bigint) --- a type of unbounded, mathematical integers
...
* [`\TYPE`](type-TYPE) --- the type of Java types; this is useful for reasoning about (compile-time) types of Java expressions.

The type parameter of the parameterized JML built-in types may be either a Java reference type or a JML built-in type, but not a primitive Java type. Use `==` for equality comparisons among values of Java or JML types.
