...
As you can see in the above, the key point is adding an invariant that describes each of the fields `x` and `y`.  (One could also add a single invariant clause, which would have in its expression the conjunction of the two invariant expressions shown, conjoined with `&&`; that is equivalent to what is shown above.) However, if only that were done, then the methods `moveRight` and `moveUp` would no longer verify, because they could violate the invariant(s), so it is necessary to change their preconditions so that the value put into the `x` or `y` fields by these methods still satisfies the invariant(s). Then when the preconditions are changed, the `test()` method no longer verifies, since the arguments to `moveRight` and `moveUp` might not satisfy the new preconditions for those methods; this explains the code added to the `test()` method.

However, something should also be said about the constructor, which has a default assignable clause of `assignable \everything`. Since the method `test()` is `static`, the instance fields `x` and `y` (i.e., `this.x` and `this.y`) are _not_ part of the constructor's pre-state. If the `test()` method was not static, then the `x` and `y` fields would be part of the method's pre-state, and so they would be potentially assigned by the call to the constructor; thus the values of these fields might not satisfy the invariant(s) after the constructor call. After the constructor call the fields of 'p' satisfy their invariants and MAX_SIZE is unchanged (since it is `final`), but at the end of test(), as would be indicated by the mention of `InvariantExit` in the error message, the invariants of the `this` object (the instance fields `this.x` and `this.y` could have been changed (by the call of the constructor) to not satisfy their invariant. This could be prevented by writing an assignable clause for the constructor (such as `assignable \nothing`) or equivalently by declaring the constructor to be `pure`. In conclusion, either the `test()` method should be `static` (so that there are no instance fields available to assign in the method's pre-state) or the constructor should be declared to be `pure` (so that it cannot assign to the instance fields of the receiver object's pre-state).
