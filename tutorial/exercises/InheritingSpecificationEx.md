...
## **Question 3**
**Specify an interface, called `Gendered` that includes a model instance field `gender` of type `String`. Your task is to write this interface's specification and to specify a `spec_pure` method `isFemale()` that returns _true_ just when the receiver (an object of a subtype of `Gendered`) is female.**
